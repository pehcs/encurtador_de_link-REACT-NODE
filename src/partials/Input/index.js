...
function Input() {
  ...
  function callAPI(url){
    axios.get(`https://api.shrtco.de/v2/shorten?url=${url}`).then(response=>{
      const { full_short_link: shortLink, original_link: originalLink } = response.data.result
  ...
  return (
    ...
    <div className="design-entrada">
      ...
      { currentLinks.map((element, index)=>{
          return (
          <ShortLinkComponent key={index} original={element.originalLink} shorted={element.shortLink}></ShortLinkComponent>
          )
        })
      }
    </div>
